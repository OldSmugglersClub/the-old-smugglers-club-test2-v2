...
function bindPagination(){['pagination-top','pagination-bottom'].forEach(id=>{const container=$(id);if(!container)return;container.onclick=event=>{const button=event.target.closest('button[data-page]');if(!button||button.disabled)return;event.preventDefault();const requested=Number.parseInt(button.dataset.page,10);if(!Number.isInteger(requested)||requested===page)return;page=requested;renderTable();$('table-panel').scrollIntoView({behavior:'smooth',block:'start'});};});}
