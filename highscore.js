...
function bindPagination(){['pagination-top','pagination-bottom'].forEach(id=>{const container=$(id);if(!container)return;container.querySelectorAll('button[data-page]').forEach(button=>{button.onclick=()=>{if(button.disabled)return;const requested=Number.parseInt(button.dataset.page,10);if(!Number.isInteger(requested)||requested===page)return;page=requested;renderTable(rows());$('table-panel').scrollIntoView({behavior:'smooth',block:'start'});};});});}
